...
const debug = require("debug")("cinco");
...
server.sockets.on("connection", function(socket) {
  ...
  socket.on("turn", function({ row, column }) {
    const game = games[socket.game];

    // Check if move is out-of-turn or illegal
    if (game.turn !== socket.pid || game.board[row][column] !== 0) {
    ...
    // Update board
    game.board[row][column] = socket.pid;
    game.captures[game.turn - 1] += game.checkCaptures([
      row,
      column
    ]);
