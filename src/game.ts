...
export type Tokens = Record<TokenID, PlayerID>;
      ...
      setState(
        produce<State>((s) => {
          ...
          if (
            ...
            hasRowVictory(s.tokens, playerID, tokenID)
          ) {
            s.status = "POST";
...
const getHasRow =
  (rowCount: number) =>
  (board: Tokens, playerID: PlayerID, tokenID: TokenID) => {
...
const getCaptures = (board: Tokens, playerID: PlayerID, tokenID: TokenID) => {
